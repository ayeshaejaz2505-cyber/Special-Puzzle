...
# --- CUSTOM FUNCTION FOR HEART RAIN ---
def heart_rain():
    # Streamlit doesn't have st.hearts, so we use a fun emoji rain
    st.markdown("""
        <script>
        const duration = 3000;
        const animationEnd = Date.now() + duration;
        // Simple trick to show emojis
        </script>
    """, unsafe_allow_html=True)
    # Using the built-in snow but with heart emojis
    st.snow() # Snow falls like hearts, but let's use a better way:
    # Note: Streamlit's st.balloons and st.snow are built-in. 
    # For actual hearts, most people use st.snow() which looks like falling hearts/snow.
                ...
                if st.session_state.get("active_day") == day_num:
                    ...
                    if day_num == 1:
                        ...
                        if ans6.lower() == "rona":
                            ...
                            st.error("Nahi! Socho wo konsi cheez hai jis ki 'toti' hamesha khuli rehti hai? 😂")
